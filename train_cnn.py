"""
CNN Model Training on NSL-KDD Dataset
Targeting 80%+ test accuracy with optimized architecture
"""
...
class ImprovedCNNClassifier(nn.Module):
    """Improved CNN with better architecture for NIDS."""
...
def main():
    # =========================================================================
    # Configuration - Optimized for 80%+ accuracy
    # =========================================================================
    CONFIG = {
        "batch_size": 512,
        "epochs": 100,
    ...
    console.print("\n[bold cyan]=" * 60)
    console.print("[bold cyan]  CNN Training for NIDS - Target: 80%+ Accuracy")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        if val_acc > best_val_acc:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), "./results/models/cnn_best.pt")
            save_marker = " ★ Saved!"
        else:
            patience_counter += 1
            save_marker = ""
    ...
    # Load best model
    model.load_state_dict(torch.load("./results/models/cnn_best.pt"))
    ...
    if test_acc >= 0.80:
        console.print("\n[bold green]🎉 TARGET ACHIEVED! Accuracy ≥ 80%[/bold green]")
    else:
        console.print(f"\n[yellow]Current accuracy: {test_acc*100:.2f}%. Target: 80%[/yellow]")
